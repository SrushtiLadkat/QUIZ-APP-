<?php
  ...
  if ($conn->connect_error) {
    ...
    $check_stmt = $conn->prepare("SELECT * FROM science WHERE username = ? OR email = ?");
    ...
    if ($result->num_rows > 0) {
      ...
      // Insert user data
      $stmt = $conn->prepare("INSERT INTO science (username, age, phone, email, password) VALUES (?, ?, ?, ?, ?)");
      // Hash the password
      ...
      echo "<script>window.location.href = 'sciencelogin.php';</script>";
