<?php
  ...
  if ($conn->connect_error) {
    ...
    $check_stmt = $conn->prepare("SELECT * FROM gk WHERE username = ? OR email = ?");
    ...
    if ($result->num_rows > 0) {
      ...
      // Insert user data
      $stmt = $conn->prepare("INSERT INTO gk (username, age, phone, email, password) VALUES (?, ?, ?, ?, ?)");
      // Hash the password
      ...
      echo "<script>window.location.href = 'gklogin.php';</script>";
